...
map:this.mainMap,title:this.config.markers[a].title,icon:this.config.markers[a].icon}),this.mainBounds.extend(new google.maps.LatLng(this.config.markers[a].position.lat,this.config.markers[a].position.lng)));a=this.config.trails.length;for(var b=0;b<this.config.trails.length;b++){var c=new Trail(this.config.trails[b],this.config.main.levelColors);c.loadTrail(function(a){a.renderTo(this.mainMap,this.onMapElemClicked.bind(this));this.mainBounds.union(a.getBounds())}.bind(this));this.trails.push(c);
console.log("Added trail "+c.getTitle());a--;0===a&&(console.log("DONE - fit map..."),this.mainMap.fitBounds(this.mainBounds,0))}this.addHelpOverlays();this.hideInfo()};MtbMapApplication.prototype.resetMainMap=function(){this.mainMap.fitBounds(this.mainBounds,0)};
...
c));b.bounds.extend(new google.maps.LatLng(a,c));f=a;h=c;var g=0;$(this).find("ele").each(function(){g=parseFloat($(this).text());null===e?e=d=g:e>g?e=g:d<g&&(d=g)});b.altitudes.push(g)})})})});b.heightDiff=d-e;a(b)}})};Trail.prototype.patchClicked=function(){this.clickCb&&this.clickCb(this)};
