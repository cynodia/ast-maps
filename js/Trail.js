    ...
    renderToMap(trackLayer, markerLayer, callback, userUpload) {
        ...
        if(!this.mapPath) {
            ...
            if(this.config.title != null) {
                if(!window.printRender && this.config.bidirectional === false) {
                    this.decoration = L.polylineDecorator(this.mapPath, {
                        patterns: [
                            {
                                offset: 0,
                                repeat: 130,
                                symbol: L.Symbol.arrowHead({
                                    ...
                                    pathOptions: {
                                        fillOpacity: 1,
                                        weight: 0,
                                        color: 'white'//this.getTrailColor()
                                    }
                                })
                            }
                        ]
                ...
                if(this.decoration) {
                    this.decoration.on('click', this.pathClicked.bind(this));
                }
...
//                                    "<br><span style=\"float:right;\"><a href=\"#\" onclick=\"openTrail(" + this.getId() + ");return false;\">Åpne</a></span><br>");
                    ...
                    if(this.decoration) {
                        this.decoration.on('mouseover', displayInfoWindow);
                        this.decoration.on('mouseout', hideInfoWindow);
                    }
                }
            }
        }
        ...
        if(this.decoration) {
            this.decoration.addTo(markerLayer);
        }
