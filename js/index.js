...
function mapUserLocation() {
    if (navigator.geolocation) {
        if(geolocationWatchId !== null) {
            application.showInfo("Oppdaterer din posisjon...", 10);
            navigator.geolocation.clearWatch(geolocationWatchId);
            geolocationWatchId = null;
        } else {
            application.showInfo("Henter din posisjon...", 10);
        }
        navigator.geolocation.getCurrentPosition(updatePosition, geolocationFail);
    } else {
        application.showInfo("Posisjondata ikke tigjengelig!", 6);
    }
}
...
function geolocationFail(error) {
    switch(error.code) {
        case error.PERMISSION_DENIED:
            application.showInfo("User denied the request for Geolocation.");
            break;
        case error.POSITION_UNAVAILABLE:
            application.showInfo("Location information is unavailable.");
            break;
        case error.TIMEOUT:
            application.showInfo("The request to get user location timed out.");
            break;
        default:
            application.showInfo("An unknown error occurred.");
            break;
    }
}
...
function updatePosition(pos) {
    ...
    if(window.mainLocationMarker) {
        ...
        if(geolocationWatchId === null) {
            application.showInfo("Posisjon oppdatert<hr>Din posisjon oppdateres også automatisk", 5);
        }
    } else {
